...
This extension is used to configure the participation factor of the generator, typically in the case of a load flow computation with distributed slack enabled (with [balance type](../simulation/loadflow/index#generic-parameters) on generator). This extension is attached to a [generator](network_subnetwork.md#generator) or a [battery](network_subnetwork.md#battery).
...
[`isReadSlackBus`](../simulation/loadflow/index#generic-parameters). Use this extension after a computation to attach
to the network the slack bus that has been selected by the load flow engine (one by connected component). You should enable
default load flow parameter [`isWriteSlackBus`](../simulation/loadflow/index#generic-parameters).
