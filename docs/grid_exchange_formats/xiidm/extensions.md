...
Note that some extensions provided by PowSyBl aren't supported in the [persistent implementation of IIDM](testlink.md).

Every extension is considered as serializable unless explicitly specified as non-serializable in XML-IIDM.

## Active power control
This extension is used to configure the participation factor of the generator, typically in the case of a load flow computation with distributed slack enabled (with [balance type](testlink.md) on generator). This extension is attached to a [generator](testlink.md#paragraph1) or a [battery](testlink.md#paragraph2).
...
Some generators can be coordinated to control reactive power in a point of the network. This extension is used to configure the percent of reactive coordinated control that comes from a generator. This extension is attached to a [generator](testlink.md).
...
This extension is attached to a [voltage level](testlink.md) and is used to define the slack bus of a power flow calculation i.e. which bus will be used to balance the active and reactive power in load flow analysis. Use this extension before a computation to force the slack bus selection. You should enable default load flow parameter [`isReadSlackBus`](testlink.md). Use this extension after a computation to attach to the network the slack bus that has been selected by the load flow engine (one by connected component). You should enable default load flow parameter [`isWriteSlackBus`](testlink.md).
...
This extension is used to model voltage control of static VAR compensators. This extension is attached to a [static VAR compensator](testlink.md).
...
The support of this feature by Load Flow implementations may vary. For example, the [OpenLoadFlow](testlink.md) implementation
...
The support of this feature by Load Flow implementations may vary. For example, the [OpenLoadFlow](testlink.md) implementation
today supports writing to the Network the terminals of the reference generators chosen via the [Reference Priority extension](testlink.md).
