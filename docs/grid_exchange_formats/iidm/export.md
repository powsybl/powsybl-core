...
**iidm.export.xml.filtered-extensions**  
The `iidm.export.xml.filtered-extensions` property is an optional property that defines the list of extensions that will not be exported by the XIIDM exporter. 
When both `iidm.export.xml.extensions` and `iidm.export.xml.filtered-extensions` contains the same extension, `iidm.export.xml.filtered-extensions` takes the priority hence the extension will not be imported.
