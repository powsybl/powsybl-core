...
Additionally, fictitious buses will be added for each three-winding transformer (the `star bus`) and boundary line (the boundary side).
...
batteries, and LCC converters connected to them. Generators with disabled voltage regulating control are considered loads
...
generators defined in the boundary bus of the boundary lines, and generators defined in the voltage source converters not exported as DC line data.
...
The next block of data in the Matpower model is the branch data. Branches are created from PowSyBl model lines, tie lines, two-winding transformers,
three-winding transformers and boundary lines. The associated branch is created only when all connected buses, or connectable buses, are exported buses.
Lines, tie lines and two-winding transformers are exported `in service` when both ends are connected. Otherwise, they are exported `out of service`.
...
When all three legs are disconnected but remain connectable to exported buses, they are exported as `out of service` branches with the star bus 
defined as type `4` (isolated bus). Boundary lines are only exported when they are connected to exported buses. Exporting a boundary line requires an
additional bus to represent the boundary side.
