...
The `loadflow-validation` module is used by the [loadflow-validation](../../user/itools/loadflow-validation.md) command and the
[load flow validation](index.md) feature. It defines the parameters used during the
validation of load flow results.

## Optional properties

(loadflow-validation-configuration-apply-reactance-correction)=
...
(loadflow-validation-configuration-check-main-component-only)=
...
(loadflow-validation-configuration-compare-results)=
### compare-results
...
(loadflow-validation-configuration-epsilon-x)=
### epsilon-x
The `epsilon-x` property is an optional property that defines the minimum value used to correct the reactance in flows validation (to avoid division per zero). The default value of this property is `0.1`.

(loadflow-validation-configuration-load-flow-name)=
### load-flow-name
...
(loadflow-validation-configuration-no-requirement-if-reactive-bound-inversion)=
...
(loadflow-validation-configuration-no-requirement-if-setpoint-outside-power-bounds)=
...
(loadflow-validation-configuration-ok-missing-values)=
### ok-missing-values
...
(loadflow-validation-configuration-output-writer)=
### output-writer
...
(loadflow-validation-configuration-table-formatter-factory)=
...
The table formatter can be configured by the [table-formatter](../../user/configuration/table-formatter.md) module.

(loadflow-validation-configuration-threshold)=
### threshold
The `threshold` property is an optional property that defines the margin used for values comparison tolerance between computed and measeured active and reactive powers. The default value of this property is `0`.

(loadflow-validation-configuration-verbose)=
### verbose
The `verbose` property is an optional property that defines whether the [loadflow-validation](../../user/itools/loadflow-validation.md) command runs in verbose or quiet mode.
