...
import com.powsybl.security.limitscaling.computation.SimpleLimitsScaler;

import java.util.Collection;
import java.util.Optional;

/**
 * <p>A limits computer that takes a single limit scaling value for all the identifiers, and applies it on each permanent and temporary limit, for pre- and post-contingency states.</p>
...
public class SimpleLimitsComputer extends AbstractLimitsComputerWithCache<Identifiable<?>, LoadingLimits> {
    ...
    public SimpleLimitsComputer(double limitScaling) {
    ...
    protected Collection<LimitsContainer<LoadingLimits>> computeUncachedLimits(Identifiable<?> identifiable, LimitType limitType, ThreeSides side, boolean monitoringOnly) {
        ...
        return groups.stream()
                .map(group -> group.getLoadingLimits(limitType)
                        .map(limits -> new SimpleLimitsScaler(limits, group.getId(), limitScaling).getLimits()))
