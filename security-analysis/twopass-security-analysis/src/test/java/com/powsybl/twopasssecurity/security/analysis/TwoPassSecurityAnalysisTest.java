 ...
 */
class TwoPassSecurityAnalysisTest {

    private Network network;
    private String workingVariantId;
    private ContingenciesProvider contingenciesProvider;
    private TwoPassSecurityAnalysis twoPassSecurityAnalysis;
    ...
    void testSwitchOnLimitViolations() {
        ...
        // 1. Launch the two-pass analysis
    ...
    @Test
    void testRequiresSecondPassWhenChangedPhaseTapChangerPresent() {
        when(contingenciesProvider.getContingencies(network)).thenReturn(Collections.singletonList(contingency1));

        List<ChangedPhaseTapChanger> changedPhaseTapChangers = Collections.singletonList(
                new ChangedPhaseTapChanger("transformer1", ThreeSides.ONE, 1, 2)
        );
