...
public abstract class AbstractSecurityAnalysisResultBuilder extends DefaultLoadFlowActionSimulatorObserver {
    ...
    public void loadFlowDiverged(RunningContext runningContext) {
        if (precontingency) {
            ...
            postContingencyResults.put(runningContext.getContingency().getId(), new PostContingencyResult(runningContext.getContingency(), PostContingencyComputationStatus.FAILED, new LimitViolationsResult(Collections.emptyList(), getPostContingencyActions(runningContext.getContingency())), NetworkResult.empty(), ConnectivityResult.empty(), Double.NaN));
        }
    }
