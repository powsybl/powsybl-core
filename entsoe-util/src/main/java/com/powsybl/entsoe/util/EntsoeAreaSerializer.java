...
public class EntsoeAreaSerializer extends AbstractExtensionSerializer<Substation, EntsoeArea> {

    public EntsoeAreaSerializer() {
        super("entsoeArea", "network", EntsoeArea.class, "entsoeArea.xsd",
                "http://www.itesla_project.eu/schema/iidm/ext/entsoe_area/1_0", "ea");
    }

    @Override
    public void write(EntsoeArea country, WriterContext context) {
        context.getWriter().writeNodeContent(country.getCode().name());
    }

    @Override
    public EntsoeArea read(Substation substation, ReaderContext context) {
