...
public class LocalLoadFlowActionSimulator extends LoadFlowActionSimulator {
    ...
    public LocalLoadFlowActionSimulator(Network network, Filtration filtration, LoadFlowActionSimulatorConfig config, boolean applyIfSolved, LoadFlowActionSimulatorObserver... observers) throws IOException {
        super(network, new LocalComputationManager(), config, applyIfSolved, observers);
        this.filtration = Objects.requireNonNull(filtration);
    }

    public LocalLoadFlowActionSimulator(Network network, Filtration filtration, LoadFlowActionSimulatorConfig config, boolean applyIfSolved, List<LoadFlowActionSimulatorObserver> observers) throws IOException {
        super(network, new LocalComputationManager(), config, applyIfSolved, observers);
        this.filtration = Objects.requireNonNull(filtration);
    }
