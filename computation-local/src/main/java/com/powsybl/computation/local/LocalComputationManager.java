...
public class LocalComputationManager implements ComputationManager {
    ...
    private ExecutionReport execute(Path workingDir, List<CommandExecution> commandExecutionList, Map<String, String> variables, ComputationOptions computationOptions, ExecutionMonitor monitor)
            ...
            IntStream.range(0, commandExecution.getExecutionCount()).forEach(idx ->
                    executionSubmitter.execute(() -> {
                        try {
                            ...
                            int exitValue = process(workingDir, commandExecution, idx, variables, computationOptions);
    ...
    private int process(Path workingDir, CommandExecution commandExecution, int executionIndex, Map<String, String> variables, ComputationOptions computationOptions) throws IOException, InterruptedException {
        Command command = commandExecution.getCommand();
        int exitValue = 0;
        long timeout = -1;
        ...
        switch (command.getType()) {
            case SIMPLE:
                ...
                timeout = computationOptions.getTimeout(simpleCmd.getId()).orElse(-1);
    ...
    public <R> CompletableFuture<R> execute(ExecutionEnvironment environment, ExecutionHandler<R> handler) {
        return execute(environment, handler, ComputationOptions.empty());
    }

    @Override
    public <R> CompletableFuture<R> execute(ExecutionEnvironment environment, ExecutionHandler<R> handler, ComputationOptions computationOptions) {
        ...
        threadPools.execute(() -> {
            ...
            try {
                try (WorkingDirectory workingDir = new WorkingDirectory(config.getLocalDir(), environment.getWorkingDirPrefix(), environment.isDebug())) {
                    ...
                    ExecutionReport report = execute(workingDir.toPath(), commandExecutionList, environment.getVariables(), computationOptions, handler::onExecutionCompletion);
