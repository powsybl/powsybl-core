...
public abstract class AbstractLoadActionSerializer<T extends AbstractLoadAction> extends StdSerializer<T> {
    ...
    public void serialize(T action, JsonGenerator jsonGenerator, SerializationContext serializationContext) throws JacksonException {
        ...
        action.getActivePowerValue().ifPresent(activePowerValue -> jsonGenerator.writeNumberProperty("activePowerValue", activePowerValue));
        action.getReactivePowerValue().ifPresent(reactivePowerValue -> jsonGenerator.writeNumberProperty("reactivePowerValue", reactivePowerValue));
