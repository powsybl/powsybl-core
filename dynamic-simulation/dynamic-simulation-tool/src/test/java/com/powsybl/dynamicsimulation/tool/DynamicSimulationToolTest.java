...
public class DynamicSimulationToolTest extends AbstractToolTest {
    ...
    public void testDynamicSimulation() throws IOException {
        ...
        // Run with curves
        assertCommand(new String[]{"dynamic-simulation", "--case-file", "/network.xiidm", "--curves-file", "/curves.groovy"}, 0, expectedOut, "");
    }

    @Test
    public void testDynamicSimulationWithCurves() throws IOException {
        // Run with curves in groovy
        assertCommand(new String[]{"dynamic-simulation", "--case-file", "/network.xiidm", "--curves-file", "/curves.groovy"}, 0, null, "");

        // Run with curves in JSON (not supported)
        assertCommand(new String[]{"dynamic-simulation", "--case-file", "/network.xiidm", "--curves-file", "/curves.json"}, 3, null, "Unsupported curves format: json");
    }
