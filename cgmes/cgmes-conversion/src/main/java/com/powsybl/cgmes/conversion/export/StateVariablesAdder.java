    ...
    private void addTapStepToCgmes() {
        ...
        for (TwoWindingsTransformer t : network.getTwoWindingsTransformers()) {
            ...
            if (hasPhaseTapChanger(t)) {
                p.put(tapChangerPositionName, is(t.getPhaseTapChanger().getTapPosition().orElseThrow(ValidationUtil::createUndefinedValueGetterException)));
                ...
                p.put(tapChangerPositionName, is(t.getRatioTapChanger().getTapPosition().orElseThrow(ValidationUtil::createUndefinedValueGetterException)));
        ...
        for (ThreeWindingsTransformer t : network.getThreeWindingsTransformers()) {
            ...
            for (Leg leg : Arrays.asList(t.getLeg1(), t.getLeg2(), t.getLeg3())) {
                if (hasPhaseTapChanger(leg)) {
                    p.put(tapChangerPositionName, is(leg.getPhaseTapChanger().getTapPosition().orElseThrow(ValidationUtil::createUndefinedValueGetterException)));
                    ...
                    p.put(tapChangerPositionName, is(leg.getRatioTapChanger().getTapPosition().orElseThrow(ValidationUtil::createUndefinedValueGetterException)));
