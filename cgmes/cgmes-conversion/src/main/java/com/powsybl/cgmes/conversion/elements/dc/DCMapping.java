...
public class DCMapping {
    ...
    public PropertyBag getDcNodeBag(String dcNodeId) {
        String nodeClass = context.nodeBreaker() ? DC_NODE : DC_TOPOLOGICAL_NODE;

        return dcNodes.stream()
                .filter(b -> b.getId(nodeClass).equals(dcNodeId))
                .findFirst()
                .orElseThrow();
    }

    public String getDcNode(String dcTerminalId) {
        if (!dcTerminalNodes.containsKey(dcTerminalId)) {
