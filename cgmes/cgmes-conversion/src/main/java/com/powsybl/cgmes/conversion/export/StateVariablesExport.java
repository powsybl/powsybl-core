    ...
    private static void writeTapSteps(Network network, String cimNamespace, XMLStreamWriter writer) throws XMLStreamException {
        for (TwoWindingsTransformer twt : network.getTwoWindingsTransformers()) {
            if (twt.hasPhaseTapChanger()) {
                ...
                writeSvTapStep(ptcId, twt.getPhaseTapChanger().getTapPosition().orElseThrow(ValidationUtil::createUndefinedValueGetterException), cimNamespace, writer);
                ...
                writeSvTapStep(rtcId, twt.getRatioTapChanger().getTapPosition().orElseThrow(ValidationUtil::createUndefinedValueGetterException), cimNamespace, writer);
            }
        }
        ...
        for (ThreeWindingsTransformer twt : network.getThreeWindingsTransformers()) {
            ...
            for (ThreeWindingsTransformer.Leg leg : Arrays.asList(twt.getLeg1(), twt.getLeg2(), twt.getLeg3())) {
                if (leg.hasPhaseTapChanger()) {
                    ...
                    writeSvTapStep(ptcId, leg.getPhaseTapChanger().getTapPosition().orElseThrow(ValidationUtil::createUndefinedValueGetterException), cimNamespace, writer);
                    ...
                    writeSvTapStep(rtcId, leg.getRatioTapChanger().getTapPosition().orElseThrow(ValidationUtil::createUndefinedValueGetterException), cimNamespace, writer);
                }
                i++;
            }
        }
