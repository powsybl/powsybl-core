...
public final class TopologyExport {
    ...
    private static void writeBoundaryLineTopologicalNodes(Network network, String cimNamespace, XMLStreamWriter writer, CgmesExportContext context) throws XMLStreamException {
        for (BoundaryLine bl : network.getBoundaryLines(BoundaryLineFilter.UNPAIRED)) {
            ...
            String topologicalNodeId = bl.getProperty(Conversion.CGMES_PREFIX_ALIAS_PROPERTIES + CgmesNames.TOPOLOGICAL_NODE_BOUNDARY);
            if (topologicalNodeId == null) {
                ...
                if (containerId == null) {
                    ...
                    LOG.error("Boundary line {}{} is not connected to a topology node in boundaries files: EQ profile must be exported for consistent results." +
                            ...
                            bl.getId(), bl.getUcteXnodeCode() != null ? " linked to X-node " + bl.getUcteXnodeCode() : "", bl.getId(), bl.getTerminal().getVoltageLevel().getId());
                ...
                bl.setProperty(Conversion.CGMES_PREFIX_ALIAS_PROPERTIES + CgmesNames.TOPOLOGICAL_NODE_BOUNDARY, fictTopologicalNodeId);
