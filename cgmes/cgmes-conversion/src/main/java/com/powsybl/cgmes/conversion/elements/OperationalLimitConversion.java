...
public class OperationalLimitConversion extends AbstractIdentifiedObjectConversion {
    ...
    private void createCurrentLimitsAdder(int terminalNumber, Branch<?> b) {
        if (terminalNumber == 1) {
            currentLimitsAdder1 = context.currentLimitsMapping().getCurrentLimitsAdder(b.getId() + "_1",
                b::newCurrentLimits1);
        } else if (terminalNumber == 2) {
            currentLimitsAdder2 = context.currentLimitsMapping().getCurrentLimitsAdder(b.getId() + "_2",
                b::newCurrentLimits2);
        } else {
            notAssigned(b);
        }
    ...
    private void createCurrentLimitsAdder(int terminalNumber, Identifiable<?> identifiable) {
        if (identifiable instanceof Line) {
            ...
            if (terminalNumber == -1) {
                context.ignored(
                    Warnings.Ignored.OPERATIONAL_LIMIT_CURRENT_FOR_XFMR,
                    CURRENT_LIMIT,
                    "Defined for Equipment TwoWindingsTransformer. Should be defined for a Terminal");
                notAssigned(b);
            } else {
                createCurrentLimitsAdder(terminalNumber, b);
            }
            ...
            currentLimitsAdder = context.currentLimitsMapping().getCurrentLimitsAdder(danglingLine.getId(),
    ...
    public boolean valid() {
        if (currentLimitsAdder == null && currentLimitsAdder1 == null && currentLimitsAdder2 == null) {
            if (terminalId != null) {
                missing(Warnings.Missing.OPERATIONAL_LIMIT_TERMINAL, String.format("Terminal %s", terminalId));
            } else if (equipmentId != null) {
                missing(Warnings.Missing.OPERATIONAL_LIMIT_EQUIPMENT, String.format("Equipment %s", equipmentId));
            }
            return false;
        }
        return true;
    ...
    public void convert() {
        ...
        if (value <= 0) {
            context.ignored(Warnings.Ignored.OPERATIONAL_LIMIT, OPERATIONAL_LIMIT, "value is <= 0");
            return;
        }
    ...
    private void addPatlCurrent(double value, CurrentLimitsAdder adder) {
        if (Double.isNaN(adder.getPermanentLimit())) {
            ...
            if (terminalId != null) {
                context.fixed(
                    Warnings.Fixes.OPERATIONAL_LIMIT_TERMINAL_PATL_MULTIPLE,
                    PERMANENT_CURRENT_LIMIT,
                    String.format("Several permanent limits defined for Terminal %s. Only the lowest is kept.",
                        terminalId));
            } else {
                context.fixed(
                    Warnings.Fixes.OPERATIONAL_LIMIT_EQUIPMENT_PATL_MULTIPLE,
                    PERMANENT_CURRENT_LIMIT,
                    String.format("Several permanent limits defined for Equipment %s. Only the lowest is kept.",
                        equipmentId));
            }
    ...
    private void addTatlCurrent(String name, double value, int acceptableDuration, CurrentLimitsAdder adder) {
        if (Double.isNaN(adder.getTemporaryLimitValue(acceptableDuration))) {
            ...
            if (terminalId != null) {
                context.fixed(
                    Warnings.Fixes.OPERATIONAL_LIMIT_TERMINAL_TATL_MULTIPLE,
                    TEMPORARY_CURRENT_LIMIT,
                    String.format(
                        "Several temporary limits defined for same acceptable duration (%d s) for Terminal %s. Only the lowest is kept.",
                        acceptableDuration, terminalId));
            } else {
                context.fixed(
                    Warnings.Fixes.OPERATIONAL_LIMIT_EQUIPMENT_TATL_MULTIPLE,
                    TEMPORARY_CURRENT_LIMIT,
                    String.format(
                        "Several temporary limits defined for same acceptable duration (%d s) for Equipment %s. Only the lowest is kept.",
    ...
    private void convertTatlCurrent(double value) {
        int acceptableDuration = p.containsKey("acceptableDuration") ? (int) p.asDouble("acceptableDuration")
            : Integer.MAX_VALUE;
        ...
        // if there is no direction, the limit is considered as absoluteValue (cf. CGMES
        // specification)
        if (direction == null || direction.endsWith("high") || direction.endsWith("absoluteValue")) {
            if (currentLimitsAdder != null) {
                addTatlCurrent(context.namingStrategy().getId("TATL", id), value, acceptableDuration,
                    currentLimitsAdder);
            } else if (currentLimitsAdder1 != null) {
                // Should we chose one terminal randomly for branches ? Here by default, we only
                // look at terminal1
                addTatlCurrent(context.namingStrategy().getId("TATL", id), value, acceptableDuration,
                    currentLimitsAdder1);
            }
        } else if (direction.endsWith("low")) {
            context.invalid(Warnings.Invalid.OPERATIONAL_LIMIT_TATL_LOW, TEMPORARY_CURRENT_LIMIT,
                String.format("TATL %s is a low limit", id));
        } else {
            context.invalid(Warnings.Invalid.OPERATIONAL_LIMIT_TATL_DIRECTION, TEMPORARY_CURRENT_LIMIT,
                String.format("TATL %s does not have a valid direction", id));
        }
    }
    ...
    private void notAssigned(Identifiable<?> eq) {
        ...
        String reason = String.format(
            ...
            subclass,
            terminalId);
        context.pending(Warnings.Pending.OPERATIONAL_LIMIT_LIMIT_NOT_ASSIGNED, OPERATIONAL_LIMIT, reason);
    }
