...
public class OperationalLimitConversion extends AbstractIdentifiedObjectConversion {
    ...
    private void convertPatl() {
        ...
        if (value <= 0) {
            context.ignored(Warnings.Ignored.OPERATIONAL_LIMIT_PATL, "Operational limit", "PATL value is <= 0");
            return;
        }
    ...
    private void convertPatlCurrent(double value) {
        ...
        if (terminal != null) {
            ...
            if (branch instanceof TwoWindingsTransformer) {
                context.ignored(Warnings.Ignored.OPERATIONAL_LIMIT_CURRENT_LIMIT, "CurrentLimit",
    ...
    private void notAssigned(Identifiable<?> eq) {
        ...
        String reason = String.format(
            ...
            subclass,
            terminal);
        context.pending(Warnings.Pending.OPERATIONAL_LIMIT_LIMIT_NOT_ASSIGNED, "Operational limit", reason);
    }
