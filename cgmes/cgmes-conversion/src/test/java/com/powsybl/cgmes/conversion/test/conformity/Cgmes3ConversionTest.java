...
public class Cgmes3ConversionTest {
    ...
    public void microGridOperationalLimits() throws IOException {
        ...
        assertEquals(1312.0, ln.getActiveCurrentLimits1().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);
        assertEquals(1312.0, ln.getActiveCurrentLimits2().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);

        assertEquals(1, (int) ln.getActiveCurrentLimits1().map(lim -> lim.getTemporaryLimits().size()).orElse(-1));
        TemporaryLimit lntl1 = ln.getActiveCurrentLimits1().flatMap(lim -> lim.getTemporaryLimits().stream().findFirst()).orElseThrow(AssertionError::new);
        assertEquals(500.0, lntl1.getValue(), 0.0);
        assertEquals(10, lntl1.getAcceptableDuration());

        assertEquals(1, (int) ln.getActiveCurrentLimits2().map(lim -> lim.getTemporaryLimits().size()).orElse(-1));
        TemporaryLimit lntl2 = ln.getActiveCurrentLimits2().flatMap(lim -> lim.getTemporaryLimits().stream().findFirst()).orElseThrow(AssertionError::new);
        ...
        assertEquals(1371.0, tln.getActiveCurrentLimits1().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);
        assertEquals(1226.0, tln.getActiveCurrentLimits2().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);

        assertEquals(1, (int) tln.getActiveCurrentLimits1().map(lim -> lim.getTemporaryLimits().size()).orElse(-1));
        TemporaryLimit tlntl1 = tln.getActiveCurrentLimits1().flatMap(lim -> lim.getTemporaryLimits().stream().findFirst()).orElseThrow(AssertionError::new);
        assertEquals(500.0, tlntl1.getValue(), 0.0);
        assertEquals(10, tlntl1.getAcceptableDuration());

        assertEquals(1, (int) tln.getActiveCurrentLimits2().map(lim -> lim.getTemporaryLimits().size()).orElse(-1));
        TemporaryLimit tlntl2 = ln.getActiveCurrentLimits2().flatMap(lim -> lim.getTemporaryLimits().stream().findFirst()).orElseThrow(AssertionError::new);
    ...
    public void miniGridOperationalLimits() throws IOException {
        ...
        assertEquals(753.0659, tw2t.getActiveCurrentLimits1().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);
        assertEquals(4123.933, tw2t.getActiveCurrentLimits2().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);

        assertEquals(0, (int) tw2t.getActiveCurrentLimits1().map(l -> l.getTemporaryLimits().size()).orElse(-1));
        assertEquals(0, (int) tw2t.getActiveCurrentLimits2().map(l -> l.getTemporaryLimits().size()).orElse(-1));

        ThreeWindingsTransformer tw3t = n.getThreeWindingsTransformer("411b5401-0a43-404a-acb4-05c3d7d0c95c");
        assertEquals(505.1817, tw3t.getLeg1().getActiveCurrentLimits().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);
        assertEquals(1683.939, tw3t.getLeg2().getActiveCurrentLimits().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);
        assertEquals(962.2509, tw3t.getLeg3().getActiveCurrentLimits().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);

        assertEquals(0, (int) tw3t.getLeg1().getActiveCurrentLimits().map(l -> l.getTemporaryLimits().size()).orElse(-1));
        assertEquals(0, (int) tw3t.getLeg2().getActiveCurrentLimits().map(l -> l.getTemporaryLimits().size()).orElse(-1));
        assertEquals(0, (int) tw3t.getLeg3().getActiveCurrentLimits().map(l -> l.getTemporaryLimits().size()).orElse(-1));
    }
    ...
    public void smallGridOperationalLimits() throws IOException {
        ...
        assertEquals(1000.0, ln.getActiveCurrentLimits1().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);
        assertEquals(1000.0, ln.getActiveCurrentLimits2().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);

        assertEquals(1, (int) ln.getActiveCurrentLimits1().map(l -> l.getTemporaryLimits().size()).orElse(-1));
        TemporaryLimit lntl1 = ln.getActiveCurrentLimits1().flatMap(l -> l.getTemporaryLimits().stream().findFirst()).orElseThrow(AssertionError::new);
        assertEquals(500.0, lntl1.getValue(), 0.0);
        assertEquals(900, lntl1.getAcceptableDuration());

        assertEquals(1, (int) ln.getActiveCurrentLimits2().map(l -> l.getTemporaryLimits().size()).orElse(-1));
        TemporaryLimit lntl2 = ln.getActiveCurrentLimits2().flatMap(l -> l.getTemporaryLimits().stream().findFirst()).orElseThrow(AssertionError::new);
        assertEquals(500.0, lntl2.getValue(), 0.0);
    ...
    public void svedalaOperationalLimits() throws IOException {
        ...
        assertEquals(2970.0, ln.getActiveCurrentLimits1().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);
        assertEquals(2970.0, ln.getActiveCurrentLimits2().map(LoadingLimits::getPermanentLimit).orElse(0.0), 0.0);
