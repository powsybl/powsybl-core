...
class CgmesTopologyKindTest extends AbstractSerDeTest {
    ...
    @ParameterizedTest
    @EnumSource(CgmesTopologyKind.class)
    ...
    private void assertValidExport(Network network, CgmesTopologyKind topologyKind, boolean cim100Export) throws IOException {
        ...
        if (topologyKind != CgmesTopologyKind.MIXED_TOPOLOGY) {
    ...
    private void assertValidCim16EquipmentOperationElements(String eqFile, String sshFile, String svFile, CgmesTopologyKind topologyKind, boolean cim100Export) {
        if (topologyKind == CgmesTopologyKind.NODE_BREAKER || cim100Export) {
            // In case of a bus branch export, the BusbarSection aren't exported and don't have a SvStatus
            int svStatusCount = topologyKind == CgmesTopologyKind.BUS_BRANCH ? 5 : 7;
            ...
            assertTrue(getElement(eqFile, "ControlArea", "network_N_CA").contains("cim:ControlArea.EnergyArea"));
            assertEquals(1, getElementCount(sshFile, "StationSupply"));
            assertEquals(1, getElementCount(sshFile, "GroundDisconnector"));
            assertEquals(svStatusCount, getElementCount(svFile, "SvStatus"));
        } else {
            ...
            assertFalse(getElement(eqFile, "ControlArea", "network_N_CA").contains("cim:ControlArea.EnergyArea"));
    ...
    private void assertValidNodeBreakerElements(String eqFile, CgmesTopologyKind topologyKind, boolean cim100Export) {
        if (topologyKind == CgmesTopologyKind.NODE_BREAKER || topologyKind == CgmesTopologyKind.MIXED_TOPOLOGY && cim100Export) {
            // In case of a CIM100 hybrid export, the buses from the BusBreakerView aren't exported as ConnectivityNode
            int connectivityNodesCount = topologyKind == CgmesTopologyKind.MIXED_TOPOLOGY ? 3 : 4;
            ...
            assertEquals(2, getElementCount(eqFile, "BusbarSection"));
        } else {
            assertEquals(0, getElementCount(eqFile, "ConnectivityNode"));
            assertEquals(0, getElementCount(eqFile, "Disconnector")); // because it is non-retained
            assertEquals(0, getElementCount(eqFile, "BusbarSection"));
        }
    }

    private void assertValidTerminalCount(String eqFile, CgmesTopologyKind topologyKind, boolean cim100Export) {
        // Generator (1), ACLineSegment (2), ConformLoad (1), NonConformLoad (1) terminals are always exported
        int terminalCount = 5;
        if (topologyKind == CgmesTopologyKind.NODE_BREAKER || cim100Export) {
            // StationSupply (1), GroundDisconnector (2) terminals are exported if not a CIM16 hybrid or bus-branch export
            terminalCount += 3;
        }
        if (topologyKind == CgmesTopologyKind.NODE_BREAKER || topologyKind == CgmesTopologyKind.MIXED_TOPOLOGY && cim100Export) {
            // BusbarSection (2), non-retained Disconnector (2) terminals are exported if not a bus-branch export
            terminalCount += 4;
        }
