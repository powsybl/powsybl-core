...
class AcDcConversionTest extends AbstractSerDeTest {
    ...
    @Test
    void skagerrakEqTest() {
        // CGMES network:
        //   2 HVDC bipoles:
        //   - SK12 is a LCC bipole, where each pole (1 & 2) is rated at 250MW/250kV.
        //   - SK34 is a mixed LCC/VSC bipole, where each pole (3 & 4) is rated at 500MW/500kV.
        //   - Poles 1, 2, 3 converters have 2 AC connections, Pole 4 converter has 1 AC connection.
        //   - The 2 bipoles share a common dedicated metallic return line which is grounded.
        ...
        Network network = readCgmesResources(IMPORT_PARAMS, DIR, "skagerrak_EQ.xml");

        // Verify all objects have been imported.
        assertEquals(26, network.getDcNodeCount());
        assertEquals(24, network.getDcSwitchCount());
        assertEquals(1, network.getDcGroundCount());
        assertEquals(5, network.getDcLineCount());
        assertEquals(2, network.getVoltageSourceConverterCount());
        assertEquals(6, network.getLineCommutatedConverterCount());
        ...
        DcNode dcNode14 = network.getDcNode("DCN_1_4");
        assertNotNull(dcNode14);
        assertEquals("DC node 1 4", dcNode14.getNameOrId());
        assertEquals(500.0, dcNode14.getNominalV());
        ...
        assertEquals("DCN_1_1", dcDisconnector.getDcNode2().getId());
        ...
        assertEquals("PT_1_1", lcc.getPccTerminal().getConnectable().getId());
        ...
        VoltageSourceConverter vsc = network.getVoltageSourceConverter("VSC_1_4");
        assertNotNull(vsc);
        assertEquals("Voltage source converter 1 4", vsc.getNameOrId());
        ...
        assertEquals("PT_1_4", vsc.getPccTerminal().getConnectable().getId());
        ...
        assertEquals("DCN_1_4P", vsc.getDcTerminal1().getDcNode().getId());
        assertEquals("DCN_1_4N", vsc.getDcTerminal2().getDcNode().getId());
        assertEquals("T_VSC_1_4_1", vsc.getAliasFromType(ALIAS_TERMINAL1).orElseThrow());
        assertEquals("T_VSC_1_4_2", vsc.getAliasFromType(ALIAS_DC_TERMINAL1).orElseThrow());
        assertEquals("T_VSC_1_4_3", vsc.getAliasFromType(ALIAS_DC_TERMINAL2).orElseThrow());
    }

    @Test
    void skagerrakSshTest() {
        // CGMES network:
        ...
        Network network = readCgmesResources(IMPORT_PARAMS, DIR, "skagerrak_EQ.xml", "skagerrak_SSH.xml");
        ...
        VoltageSourceConverter vsc2 = network.getVoltageSourceConverter("VSC_2_4");
