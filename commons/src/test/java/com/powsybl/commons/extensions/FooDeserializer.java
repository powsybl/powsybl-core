...
public class FooDeserializer extends StdDeserializer<Foo> {
    ...
    static Foo read(InputStream stream, ReportNode reportNode) throws JacksonException {
        SimpleModule module = new SimpleModule();
    ...
    static Foo update(InputStream stream, Foo foo, ReportNode reportNode) throws JacksonException {
        SimpleModule module = new SimpleModule();
        module.addDeserializer(Foo.class, new FooDeserializer(reportNode));
        JsonMapper mapper = JsonUtil.createJsonMapperBuilder()
            .addModule(module)
