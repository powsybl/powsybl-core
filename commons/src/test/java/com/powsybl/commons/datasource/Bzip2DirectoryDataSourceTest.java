...
public class Bzip2DirectoryDataSourceTest extends DirectoryDataSourceTest {
    ...
    protected boolean appendTest() {
        return false; // FIXME append does not work with bzip2 compression
    }

    static Stream<Arguments> provideArgumentsForWriteThenReadTest() {
        return Stream.of(
            Arguments.of("foo", ".iidm", null, CompressionFormat.BZIP2),
            Arguments.of("foo", "", null, CompressionFormat.BZIP2),
            Arguments.of("foo", ".v3", null, CompressionFormat.BZIP2)
        );
    }
