 ...
 */
public final class ConnectableSerializerUtil {
    ...
    private ConnectableSerializerUtil() {
    }
    ...
    public static boolean hasValidOperationalLimits(Branch<?> branch, NetworkSerializerWriterContext context) {
    ...
    public static boolean hasValidOperationalLimits(FlowsLimitsHolder limitsHolder, NetworkSerializerWriterContext context) {
        if (context.getVersion().compareTo(IidmVersion.V_1_5) >= 0) {
    ...
    public static void writeNodeOrBus(Integer index, Terminal t, NetworkSerializerWriterContext context) {
        if (index != null) {
    ...
    private static void writeNode(Integer index, Terminal t, NetworkSerializerWriterContext context) {
        context.getWriter().writeIntAttribute(NODE + indexToString(index), t.getNodeBreakerView().getNode());
    }

    private static void writeBus(Integer index, Bus bus, Bus connectableBus, NetworkSerializerWriterContext context) {
        if (bus != null) {
    ...
    public static void readNodeOrBus(InjectionAdder<?, ?> adder, NetworkSerializerReaderContext context) {
        readNodeOrBus(adder, "", context);
    }

    public static void readNodeOrBus(InjectionAdder<?, ?> adder, String suffix, NetworkSerializerReaderContext context) {
    ...
    public static void readNodeOrBus(BranchAdder<?, ?> adder, NetworkSerializerReaderContext context) {
    ...
    public static void readNodeOrBus(int index, LegAdder adder, NetworkSerializerReaderContext context) {
    ...
    private static <L extends LoadingLimits> void writeLoadingLimits(Integer index, L limits, TreeDataWriter writer, String nsUri, IidmVersion version,
        ...
        if (!Double.isNaN(limits.getPermanentLimit())
            ...
            for (LoadingLimits.TemporaryLimit tl : IidmSerializerUtil.sortedTemporaryLimits(limits.getTemporaryLimits(), exportOptions)) {
