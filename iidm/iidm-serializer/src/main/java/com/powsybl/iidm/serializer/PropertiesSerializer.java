...
public final class PropertiesSerializer {
    ...
    public static void write(Identifiable<?> identifiable, NetworkSerializerWriterContext context) {
        if (identifiable.hasProperty()) {
            context.getWriter().writeStartNodes(ARRAY_ELEMENT_NAME);
            for (String name : IidmSerializerUtil.sortedNames(identifiable.getPropertyNames(), context.getOptions())) {
    ...
    public static void read(Identifiable identifiable, NetworkSerializerReaderContext context) {
