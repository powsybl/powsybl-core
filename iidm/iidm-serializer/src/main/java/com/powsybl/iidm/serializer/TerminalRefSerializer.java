 ...
 */
public final class TerminalRefSerializer {

    private static final String ID = "id";
    private static final String SIDE = "side";

    public static void writeTerminalRef(Terminal t, NetworkSerializerWriterContext context, String elementName) {
        writeTerminalRef(t, context, context.getVersion().getNamespaceURI(context.isValid()), elementName);
    }

    public static void writeTerminalRef(Terminal t, NetworkSerializerWriterContext context, String namespace, String elementName) {
        writeTerminalRef(t, context, namespace, elementName, context.getWriter());
    }

    public static void writeTerminalRef(Terminal t, NetworkSerializerWriterContext context, String namespace, String elementName, TreeDataWriter writer) {
        writer.writeStartNode(namespace, elementName);
        writeTerminalRefAttribute(t, context, writer);
        writer.writeEndNode();
    }

    public static void writeTerminalRefAttribute(Terminal t, NetworkSerializerWriterContext context) {
        writeTerminalRefAttribute(t, context, context.getWriter());
    }

    public static void writeTerminalRefAttribute(Terminal t, NetworkSerializerWriterContext context, TreeDataWriter writer) {
    ...
    public static Terminal readTerminal(NetworkSerializerReaderContext context, Network n) {
        String id = context.getAnonymizer().deanonymizeString(context.getReader().readStringAttribute(ID));
        String side = context.getReader().readStringAttribute(SIDE);
        context.getReader().readEndNode();
        return TerminalRefSerializer.resolve(id, side, n);
    }

    public static void readTerminalRef(NetworkSerializerReaderContext context, Network network, Consumer<Terminal> endTaskTerminalConsumer) {
        String id = context.getAnonymizer().deanonymizeString(context.getReader().readStringAttribute(ID));
        String side = context.getReader().readStringAttribute(SIDE);
        context.getReader().readEndNode();
    ...
    public static Terminal resolve(String id, String sideText, Network network) {
        ...
        return TerminalRefSerializer.resolve(id, side, network);
    }
