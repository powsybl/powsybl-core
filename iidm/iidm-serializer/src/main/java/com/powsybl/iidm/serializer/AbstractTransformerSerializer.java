 ...
 */
abstract class AbstractTransformerSerializer<T extends Connectable<T>, A extends IdentifiableAdder<T, A>> extends AbstractSimpleIdentifiableSerializer<T, A, Substation> {
    ...
    private static double readTargetDeadband(NetworkSerializerReaderContext context, boolean regulating) {
        ...
        IidmSerializerUtil.runFromMinimumVersion(IidmVersion.V_1_2,
                context, () -> targetDeadband[0] = context.getReader().readDoubleAttribute(TARGET_DEADBAND));
        return targetDeadband[0];
    }

    private static void writeTapChanger(TapChanger<?, ?> tc, NetworkSerializerWriterContext context) {
    ...
    protected static void writeRatioTapChanger(String name, RatioTapChanger rtc, NetworkSerializerWriterContext context) {
        ...
        if (rtc.getRegulationTerminal() != null) {
            TerminalRefSerializer.writeTerminalRef(rtc.getRegulationTerminal(), context, ELEM_TERMINAL_REF);
        }
    ...
    protected static void readRatioTapChanger(String elementName, RatioTapChangerAdder adder, Terminal terminal, NetworkSerializerReaderContext context) {
        ...
        context.getReader().readChildNodes(subElementName -> {
            switch (subElementName) {
                case ELEM_TERMINAL_REF -> {
                    ...
                    TerminalRefSerializer.readTerminalRef(context, terminal.getVoltageLevel().getNetwork(), tRef -> {
    ...
    protected static void readRatioTapChanger(TwoWindingsTransformer twt, NetworkSerializerReaderContext context) {
        readRatioTapChanger(RATIO_TAP_CHANGER, twt.newRatioTapChanger(), twt.getTerminal1(), context);
    }

    protected static void readRatioTapChanger(int leg, ThreeWindingsTransformer.Leg twl, NetworkSerializerReaderContext context) {
        readRatioTapChanger(RATIO_TAP_CHANGER + leg, twl.newRatioTapChanger(), twl.getTerminal(), context);
    }

    protected static void writePhaseTapChanger(String name, PhaseTapChanger ptc, NetworkSerializerWriterContext context) {
        context.getWriter().writeStartNode(context.getVersion().getNamespaceURI(context.isValid()), name);
        ...
        if (ptc.getRegulationTerminal() != null) {
            TerminalRefSerializer.writeTerminalRef(ptc.getRegulationTerminal(), context, ELEM_TERMINAL_REF);
        }
    ...
    protected static void readPhaseTapChanger(String name, PhaseTapChangerAdder adder, Terminal terminal, NetworkSerializerReaderContext context) {
        boolean regulating = context.getReader().readBooleanAttribute(ATTR_REGULATING, false);
        int lowTapPosition = context.getReader().readIntAttribute(ATTR_LOW_TAP_POSITION);
        Integer tapPosition = context.getReader().readIntAttribute(ATTR_TAP_POSITION);
        double targetDeadband = readTargetDeadband(context, regulating);
    ...
    protected static void readPhaseTapChanger(TwoWindingsTransformer twt, NetworkSerializerReaderContext context) {
        readPhaseTapChanger(PHASE_TAP_CHANGER, twt.newPhaseTapChanger(), twt.getTerminal1(), context);
    }

    protected static void readPhaseTapChanger(int leg, ThreeWindingsTransformer.Leg twl, NetworkSerializerReaderContext context) {
        readPhaseTapChanger(PHASE_TAP_CHANGER + leg, twl.newPhaseTapChanger(), twl.getTerminal(), context);
    }

    private static void readSteps(NetworkSerializerReaderContext context, StepConsumer consumer) {
     ...
     */
    protected static void readRatedS(String name, NetworkSerializerReaderContext context, DoubleConsumer consumer) {
     ...
     */
    protected static void writeRatedS(String name, double ratedS, NetworkSerializerWriterContext context) {
        IidmSerializerUtil.runFromMinimumVersion(IidmVersion.V_1_2, context, () -> context.getWriter().writeDoubleAttribute(name, ratedS));
    }
}
