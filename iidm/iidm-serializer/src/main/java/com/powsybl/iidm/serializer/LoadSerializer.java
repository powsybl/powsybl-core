 ...
 */
class LoadSerializer extends AbstractComplexIdentifiableSerializer<Load, LoadAdder, VoltageLevel> {

    static final LoadSerializer INSTANCE = new LoadSerializer();
    ...
    @Override
    protected void writeRootElementAttributes(Load l, VoltageLevel vl, NetworkSerializerWriterContext context) {
    ...
    @Override
    protected void writeSubElements(Load load, VoltageLevel parent, NetworkSerializerWriterContext context) {
        load.getModel().ifPresent(model -> {
            IidmSerializerUtil.assertMinimumVersion(ROOT_ELEMENT_NAME, MODEL, IidmSerializerUtil.ErrorMessage.NOT_NULL_NOT_SUPPORTED, IidmVersion.V_1_10, context);
            IidmSerializerUtil.runFromMinimumVersion(IidmVersion.V_1_10, context, () -> writeModel(model, context));
        });
    }

    private void writeModel(LoadModel model, NetworkSerializerWriterContext context) {
    ...
    @Override
    protected void readRootElementAttributes(LoadAdder adder, List<Consumer<Load>> toApply, NetworkSerializerReaderContext context) {
    ...
    @Override
    protected void readSubElements(String id, LoadAdder adder, List<Consumer<Load>> toApply, NetworkSerializerReaderContext context) {
        context.getReader().readChildNodes(elementName -> {
            switch (elementName) {
                case EXPONENTIAL_MODEL -> {
                    IidmSerializerUtil.assertMinimumVersion(ROOT_ELEMENT_NAME, EXPONENTIAL_MODEL, IidmSerializerUtil.ErrorMessage.NOT_SUPPORTED, IidmVersion.V_1_10, context);
