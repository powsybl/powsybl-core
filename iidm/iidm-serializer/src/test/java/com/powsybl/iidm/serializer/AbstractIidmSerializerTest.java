...
public abstract class AbstractIidmSerializerTest extends AbstractSerializerTest {
    ...
    protected void roundTripVersionedXmlTest(String file, IidmVersion... versions) throws IOException {
        for (IidmVersion version : versions) {
            roundTripXmlTest(NetworkSerializer.read(getVersionedNetworkAsStream(file, version)),
    ...
    protected void roundTripVersionedJsonTest(String file, IidmVersion... versions) throws IOException {
        for (IidmVersion version : versions) {
            ...
            roundTripTest(NetworkSerializer.read(getVersionedNetworkAsStream(file, version), options, null),
                    (n, xmlFile) -> NetworkSerializer.write(n, new ExportOptions().setVersion(version.toString(".")).setFormat(TreeDataFormat.JSON), xmlFile),
                    xmlFile -> NetworkSerializer.read(xmlFile, options),
                    getVersionedNetworkPath(file, version));
        }
    }
    ...
    private static BiConsumer<Network, Path> writeAndValidate(IidmVersion version) {
        ...
        return (n, p) -> NetworkSerializer.writeAndValidate(n, options, p);
    }
}
