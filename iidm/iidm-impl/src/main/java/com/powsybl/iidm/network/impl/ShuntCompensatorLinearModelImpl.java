...
class ShuntCompensatorLinearModelImpl implements ShuntCompensatorModelExt, ShuntCompensatorLinearModel {
    ...
    public ShuntCompensatorLinearModel setMaximumSectionCount(int maximumSectionCount) {
        ValidationUtil.checkSections(shuntCompensator, shuntCompensator.getSectionCount().isPresent() ? shuntCompensator.getSectionCount().getAsInt() : null, maximumSectionCount, shuntCompensator.getNetwork().getMinValidationLevel());
