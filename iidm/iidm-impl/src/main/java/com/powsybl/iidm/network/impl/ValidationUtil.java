...
public final class ValidationUtil {
    ...
    static void checkShuntCompensatorRegulation(Validable validable, boolean regulating, double targetV, double targetDeadband) {
        if (targetDeadband < 0) {
        ...
        if (!Double.isNaN(targetV) && targetV < 0) {
            throw createInvalidValueException(validable, targetV, "targetV", "targetV is strictly negative");
        }
        if (regulating) {
            if (Double.isNaN(targetV) || targetV == 0) {
