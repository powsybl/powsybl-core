...
public class ManipulationsOnStatesTest {
    ...
    public void baseTests() {
        ...
        assertEquals(Sets.newHashSet(VariantManagerConstants.INITIAL_VARIANT_ID, "s1", "s2", "s3", "s4"), variantManager.getStateIds());

        // delete
        variantManager.removeVariant("s2");
        assertEquals(Sets.newHashSet(VariantManagerConstants.INITIAL_VARIANT_ID, "s1", "s3", "s4"), variantManager.getStateIds());

        // allocate
        variantManager.cloneVariant("s4", "s2b");
        assertEquals(Sets.newHashSet(VariantManagerConstants.INITIAL_VARIANT_ID, "s1", "s2b", "s3", "s4"), variantManager.getStateIds());

        // reduce
        variantManager.setWorkingVariant("s4");
        variantManager.removeVariant("s4");
        assertEquals(Sets.newHashSet(VariantManagerConstants.INITIAL_VARIANT_ID, "s2b", "s1", "s3"), variantManager.getStateIds());
