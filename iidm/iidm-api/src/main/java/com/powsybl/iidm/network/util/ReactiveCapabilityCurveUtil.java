...
public final class ReactiveCapabilityCurveUtil {
    ...
    public static ReactiveCapabilityCurve.Point extrapolateReactiveLimitsSlope(double p, TreeMap<Double, ReactiveCapabilityCurve.Point> points,
        ...
        if (minQ <= maxQ) {
            ...
            LOGGER.warn("Extrapolation of reactive capability curve for {} leads to minQ > maxQ, correcting to minQ = maxQ",
                ownerDescription); // This log message can be over flowing (if called at each iteration), apply filters in logback to avoid it
