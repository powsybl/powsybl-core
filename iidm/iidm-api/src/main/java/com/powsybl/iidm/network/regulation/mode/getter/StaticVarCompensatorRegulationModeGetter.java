 ...
 */
public class StaticVarCompensatorRegulationModeGetter implements RegulationModeGetter {
    @Override
    public boolean supports(Class<?> holderClass) {
        return holderClass == StaticVarCompensator.class;
    }

    @Override
    public Set<RegulationMode> getAllowedRegulationModes(boolean isRemoteRegulating) {
        return Set.of(VOLTAGE, REACTIVE_POWER, VOLTAGE_PER_REACTIVE_POWER);
    }
}
