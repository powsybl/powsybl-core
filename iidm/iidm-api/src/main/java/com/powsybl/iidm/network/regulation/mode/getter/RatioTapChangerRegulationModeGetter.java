 ...
 */
public class RatioTapChangerRegulationModeGetter implements RegulationModeGetter {
    @Override
    public boolean supports(Class<?> holderClass) {
        return RatioTapChanger.class == holderClass;
    }

    @Override
    public Set<RegulationMode> getAllowedRegulationModes(boolean isRemoteRegulating) {
        return isRemoteRegulating ? Set.of(VOLTAGE, REACTIVE_POWER) : Set.of();
    }
}
