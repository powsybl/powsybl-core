...
public final class ValidationUtil {
    ...
    public static ValidationLevel checkRatioTapChangerRegulation(Validable validable, Boolean regulating, boolean loadTapChangingCapabilities,
        ...
        if (regulating == null) {
            throwExceptionOrLogError(validable, "Undefined regulating status", throwException, reporter);
            validationLevel = ValidationLevel.min(validationLevel, ValidationLevel.SCADA);
        }
        if (regulating != null && regulating) {
    ...
    public static ValidationLevel checkPhaseTapChangerRegulation(Validable validable, PhaseTapChanger.RegulationMode regulationMode,
        ...
        if (regulating == null) {
            throwExceptionOrLogError(validable, "phase regulating status is not set", throwException, reporter);
            validationLevel = ValidationLevel.min(validationLevel, ValidationLevel.SCADA);
        }
        if (regulating != null && regulating && regulationMode != null) {
    ...
    private static ValidationLevel checkIdentifiable(Identifiable<?> identifiable, ValidationLevel previous, boolean throwException, Reporter reporter) {
        ...
        if (identifiable instanceof Validable) {
            ...
            if (identifiable instanceof Battery) {
                ...
                validationLevel = ValidationLevel.min(validationLevel, checkVoltageControl(validable, generator.findVoltageRegulatorStatus().orElse(null), generator.getTargetV(), generator.getTargetQ(), throwException, reporter));
                ...
                validationLevel = ValidationLevel.min(validationLevel, checkVoltageControl(validable, shunt.isVoltageRegulatorOn().orElse(null), shunt.getTargetV(), throwException, reporter));
                validationLevel = ValidationLevel.min(validationLevel, checkTargetDeadband(validable, "shunt compensator", shunt.isVoltageRegulatorOn().orElse(null), shunt.getTargetDeadband(), throwException, reporter));
