 ...
 */
public class GeneratorRegulationModeGetter implements RegulationModeGetter {
    @Override
    public boolean supports(Class<?> holderClass) {
        return holderClass == Generator.class;
    }

    @Override
    public Set<RegulationMode> getAllowedRegulationModes(boolean isRemoteRegulating) {
        return isRemoteRegulating ? Set.of(VOLTAGE, REACTIVE_POWER) : Set.of(VOLTAGE); // REACTIVE_POWER_PER_ACTIVE_POWER not yet supported
    }
}
