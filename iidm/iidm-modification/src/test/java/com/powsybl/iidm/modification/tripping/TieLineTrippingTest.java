...
class TieLineTrippingTest extends AbstractTrippingTest {
    ...
    void tieLineTrippingTest() {
        ...
        assertTrue(line.getBoundaryLine1().getTerminal().isConnected());
        assertTrue(line.getBoundaryLine2().getTerminal().isConnected());

        new TieLineTripping("NHV1_NHV2_1", "VLHV2").apply(network);

        assertTrue(line.getBoundaryLine1().getTerminal().isConnected());
        assertFalse(line.getBoundaryLine2().getTerminal().isConnected());

        new TieLineTripping("NHV1_NHV2_1").apply(network);

        assertFalse(line.getBoundaryLine1().getTerminal().isConnected());
        assertFalse(line.getBoundaryLine2().getTerminal().isConnected());
