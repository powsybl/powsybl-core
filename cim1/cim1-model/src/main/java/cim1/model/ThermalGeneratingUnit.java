 ...
 * Description :
 *
 * This class "ThermalGeneratingUnit.java" represents the
 * class ThermalGeneratingUnit extends GeneratingUnit
 *    + FossilFuels   1..1   FossilFuel 

 */
public class ThermalGeneratingUnit extends GeneratingUnit {

    private static Logger LOGGER = LoggerFactory.getLogger(ThermalGeneratingUnit.class);

    /**
     * Attribute "fossilFuels"
     *
    ...
    */
    ...
    /**
     * Utility to return the "fossilFuels"
     *
     * @return the value of the attribute "fossilFuels"
     */
    public FossilFuel getFossilFuels() {
        return this.fossilFuels;
    }

    /**
     * Utility to test if the value of "fossilFuels" has been set
     *
     * @return boolean
     *            if true the attribute "fossilFuels" is already set
     *            if false it isn't yet
     */
    public boolean fossilFuelsIsSet() {
        return this.currentBitset.get(0);
    }

    /**
     * Utility to set the "fossilFuelsValue"
     *
     * @param fossilFuelsValue
     *            instance to set
     * @param setInverse
     ...
     */
    public void setFossilFuels(
            FossilFuel fossilFuelsValue,
            boolean setInverse) throws LinkageException {
        this.fossilFuels = fossilFuelsValue;
        this.currentBitset.set(0);
        if (setInverse) {
            
            if (fossilFuelsValue != null) {
                fossilFuelsValue.setThermalGeneratingUnit(this, false);
            }
        }
    }
     ...
     *
     * @param id
     *            id of the class ThermalGeneratingUnit to create
     * @return the class created
     */
    public static ThermalGeneratingUnit create(final String id) {
        ThermalGeneratingUnit newThermalGeneratingUnit = new ThermalGeneratingUnit();
        newThermalGeneratingUnit.setId(id);
        return newThermalGeneratingUnit;
    }
    ...
    public void readAttribute(final String class_name, final String attr_name,
        ...
        if (class_name.equals("ThermalGeneratingUnit")) {
    ...
    public void write(XMLStreamWriter xmlsw, boolean writeID) {
        ...
        if (writeID) {
            ...
            try {
                xmlsw.writeStartElement(CIMURI.CIMURI, "ThermalGeneratingUnit");
    ...
    public void resolveLinks(CIMModel model, CIMModel boundaryModel) throws LinkageException {
        if (idFossilFuels != null) {
            FossilFuel attributeToSet =
                    model.searchFossilFuel
                    (idFossilFuels);
            ...
            if (attributeToSet != null) {
                ...
                try {
                    this.setFossilFuels(attributeToSet, setInverse);
                } catch(LinkageException e) {
                   LOGGER.error(e.toString(), e);
                    throw new LinkageException(e.getMessage());
                }
            } else if (boundaryModel != null) {
                FossilFuel attributeToSetFromBoundary = boundaryModel.searchFossilFuel(idFossilFuels);
                if(attributeToSetFromBoundary != null) {
                    attributeToSetFromBoundary.setFromBoundary(true);
                    try {
                        model.createFossilFuel(idFossilFuels, attributeToSetFromBoundary);
                    } catch (InterpretationException e) {
                       LOGGER.error(e.toString(), e);
                    }
                    boolean setInverse = true;
                    try {
                        this.setFossilFuels(attributeToSetFromBoundary, setInverse);
                    } catch(LinkageException e) {
                       LOGGER.error(e.toString(), e);
                        throw new LinkageException(e.getMessage());
                    }
                    ...
                    StringBuilder errorMessage = new StringBuilder(
                            "Could not find the reference to object of type \"FossilFuel\" and id \"");
                    errorMessage.append(idFossilFuels);
                    errorMessage.append("\" in \"ThermalGeneratingUnit\" of id \"");
     ...
     *
     * @param otherThermalGeneratingUnit
     *            the instance to copy
     */
    public void copyBase(final ThermalGeneratingUnit otherThermalGeneratingUnit) {
        ...
        if (otherThermalGeneratingUnit.currentBitset.get(0)) {
            FossilFuel fossilFuelsAssociation = otherThermalGeneratingUnit.getFossilFuels();

            //this.idFossilFuels = (fossilFuelsAssociation != null) ? fossilFuelsAssociation.getId() : "";
            if (fossilFuelsAssociation != null) {
                this.idFossilFuels = fossilFuelsAssociation.getId();
     ...
     *
     * @param ThermalGeneratingUnit
     *            the reference to the class to copy
     * @return the class constructed
     */
    private ThermalGeneratingUnit(final ThermalGeneratingUnit otherThermalGeneratingUnit) {
        // TODO : check exception safe
        this.copyBase(otherThermalGeneratingUnit);
    }

   /**
    * Constructor of the class ThermalGeneratingUnit
    */
     ...
     */
    public ThermalGeneratingUnit clone() {
        ThermalGeneratingUnit newInstance = new ThermalGeneratingUnit(this);
        ThermalGeneratingUnit newInstanceSP = newInstance;
