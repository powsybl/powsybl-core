...
/**
 * Sparse matrix LU decomposition using RTE RLU factorization.
 *
 * <p>This implementation delegates to the {@code rtemath} native library
 * (powsybl-rte-math-native), which wraps the RTE RLU sparse factorization.
 * It can be used as an alternative to {@link SparseLUDecomposition} (KLU-based)
 * to compare correctness and performance of the two implementations.</p>
 *
 * @see SparseMatrix
 *
 * @author Geoffroy Jamgotchian {@literal <geoffroy.jamgotchian at rte-france.com>}
 */
class SparseRLUDecomposition implements LUDecomposition {

    private static final Logger LOGGER = LoggerFactory.getLogger(SparseRLUDecomposition.class);
    ...
    static {
        try {
            NativeLoader.loadLibrary("rtemath");
        } catch (IOException e) {
            throw new UncheckedIOException(e);
        }
    }

    SparseRLUDecomposition(SparseMatrix matrix) {
    ...
    private void init(String id, SparseMatrix matrix) {
        ...
        stopwatch.stop();
        LOGGER.debug("Sparse RLU decomposition done in {} us", stopwatch.elapsed(TimeUnit.MICROSECONDS));
    }
    ...
    public void update(boolean allowIncrementalUpdate) {
        ...
        stopwatch.stop();
        LOGGER.debug("Sparse RLU decomposition updated (refactor rgrowth is {}, threshold is {}) in {} us",
