...
% if seed is zero, 'shuffle'  on current platform time    
if (s_rng_seed == 0)
    rng('shuffle','twister');
    rng_data = rng;
    disp(sprintf(' s_rng_seed is 0: shuffle on current platform time -> set to %d', rng_data.Seed));
else 
